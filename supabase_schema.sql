...
\restrict yxzl6g4fVBWGBwHfRqCcZYEITav0TmSKqMYrCBrWaBICyWeY9VvXc56PK9oUTzk
...
declare
  ...
  visits_count int;
begin
  -- 1. حساب إجمالي المبيعات (للطلبات غير الملغية)
  select coalesce(sum(total_amount), 0) 
  from orders 
  where status != 'cancelled' 
  into total_revenue;

  -- 2. حساب عدد الطلبات الجديدة (التي تحتاج معالجة)
  select count(*) 
  from orders 
  where status = 'new' 
  into new_orders_count;

  -- 3. حساب عدد العملاء (باستخدام جدول profiles المستخدم في شاشة العملاء داخل لوحة التحكم)
  select count(*) from profiles into clients_count;
 
  -- 4. حساب عدد المنتجات
  select count(*) from products into products_count;

  -- 5. حساب عدد زيارات الموقع (من جدول events)
  select count(*)
  from events
  where name = 'home_visit'
  into visits_count;
...
--
-- Name: get_weekly_sales(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_weekly_sales() RETURNS json
    LANGUAGE plpgsql
    AS $$
declare
  result json;
begin
  ...
  from (
    select 
      to_char(created_at, 'Dy') as day_name, -- اسم اليوم (Mon, Tue...)
      sum(total_amount) as total
    from orders
    where created_at > now() - interval '7 days'
      and status != 'cancelled' -- تجاهل الملغي
    group by day_name, date_trunc('day', created_at)
    order by date_trunc('day', created_at)
  ) t;
  
  return coalesce(result, '[]'::json); -- إرجاع مصفوفة فارغة لو لم توجد مبيعات
end;
$$;
...
begin
  ...
  values (
    ...
    new.email,
    new.raw_user_meta_data->>'phone' -- إذا كنت تمرر الهاتف عند التسجيل
  );
  return new;
end;
$$;
...
CREATE FUNCTION public.update_client_stats() RETURNS trigger
    ...
    AS $$
BEGIN
  -- نحدث بيانات العميل بناءً على رقم الهاتف
  IF (TG_OP = 'INSERT') THEN
    UPDATE public.profiles 
    SET total_spent = total_spent + NEW.total_amount, total_orders = total_orders + 1
    WHERE phone = NEW.customer_phone;
  ELSIF (TG_OP = 'UPDATE' AND OLD.status != 'cancelled' AND NEW.status = 'cancelled') THEN
    -- إذا تم إلغاء الطلب، نخصم قيمته
    UPDATE public.profiles 
    SET total_spent = total_spent - OLD.total_amount, total_orders = total_orders - 1
    WHERE phone = OLD.customer_phone;
  END IF;
  RETURN NEW;
END;
$$;
...
begin
  ...
  set 
    -- نستخدم coalesce لتحويل القيمة null إلى 0 في حال حذف كل التقييمات
    rating_average = coalesce((select avg(rating) from public.reviews where product_id = new.product_id), 0),
...
--
-- Name: verify_and_apply_coupon(text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.verify_and_apply_coupon(p_code text, p_phone text) RETURNS json
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_coupon record;
...
BEGIN
  ...
  IF v_coupon IS NULL THEN
    RETURN json_build_object('valid', false, 'message', 'الكوبون غير موجود أو غير مفعل');
  END IF;

  -- التحقق من التاريخ
  IF v_coupon.expiration_date IS NOT NULL AND v_coupon.expiration_date < NOW() THEN
    RETURN json_build_object('valid', false, 'message', 'الكوبون منتهي الصلاحية');
  END IF;

  -- التحقق من الحد الأقصى
  IF v_coupon.used_count >= v_coupon.usage_limit THEN
    RETURN json_build_object('valid', false, 'message', 'تم استنفاد عدد مرات استخدام هذا الكوبون');
  END IF;

  -- التحقق من استخدام العميل السابق (إذا وجد هاتف)
  IF p_phone IS NOT NULL AND p_phone != '' THEN
    SELECT id INTO v_usage_check FROM public.coupon_usage 
    WHERE coupon_id = v_coupon.id AND customer_phone = p_phone LIMIT 1;
    
    IF v_usage_check IS NOT NULL THEN
      RETURN json_build_object('valid', false, 'message', 'لقد استخدمت هذا الكوبون مسبقاً');
    END IF;
  END IF;

  -- الكوبون صالح! نرجع بياناته
  RETURN json_build_object(
    'valid', true, 
...
CREATE TABLE public.app_settings (
    ...
    whatsapp_number text DEFAULT '962790000000'::text,
    facebook_url text,
    instagram_url text,
    tiktok_url text,
    owner_name text DEFAULT 'د. آدم'::text,
    owner_bio text DEFAULT 'خبرة 10 سنوات في مجال الراحة والنوم الصحي.'::text,
...
CREATE TABLE public.banners (
    ...
    button_text text DEFAULT 'تسوق الآن'::text,
    link_target text DEFAULT '/category/bedding'::text,
    text_color text DEFAULT '0xFFFFFFFF'::text,
...
CREATE TABLE public.categories (
    ...
    color_value bigint DEFAULT '4278855239'::bigint,
    is_active boolean DEFAULT true,
    sort_order integer DEFAULT 0
);
...
CREATE TABLE public.coupons (
    ...
    is_active boolean DEFAULT true,
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    CONSTRAINT chk_coupons_discount_type CHECK (((discount_type IS NULL) OR (discount_type = ANY (ARRAY['fixed'::text, 'percent'::text])))),
...
CREATE TABLE public.orders (
    ...
    user_id uuid,
    CONSTRAINT chk_orders_platform CHECK (((platform IS NULL) OR (platform = ANY (ARRAY['whatsapp'::text, 'whatsapp_quick'::text, 'web'::text, 'mobile'::text])))),
    CONSTRAINT chk_orders_status CHECK (((status IS NULL) OR (status = ANY (ARRAY['new'::text, 'processing'::text, 'completed'::text, 'cancelled'::text]))))
);
...
ALTER TABLE public.support_tickets ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
...
ALTER TABLE ONLY public.seo_pages
    ADD CONSTRAINT seo_pages_pkey PRIMARY KEY (key);
...
ALTER TABLE ONLY public.static_pages
    ADD CONSTRAINT static_pages_pkey PRIMARY KEY (key);
...
--
-- Name: wishlist wishlist_user_email_product_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.wishlist
    ADD CONSTRAINT wishlist_user_email_product_id_key UNIQUE (user_email, product_id);
...
--
-- Name: favorites_user_product_uniq; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX favorites_user_product_uniq ON public.favorites USING btree (user_id, product_id);


--
-- Name: idx_coupon_usage_coupon_phone; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_coupon_usage_coupon_phone ON public.coupon_usage USING btree (coupon_id, customer_phone);
...
--
-- Name: idx_events_name_created_at; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_events_name_created_at ON public.events USING btree (name, created_at);
...
--
-- Name: idx_products_title_search; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_products_title_search ON public.products USING gin (title public.gin_trgm_ops);
...
--
-- Name: products_description_search_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX products_description_search_idx ON public.products USING gin (description public.gin_trgm_ops);
...
CREATE INDEX products_title_search_idx ON public.products USING gin (title public.gin_trgm_ops);
...
--
-- Name: orders on_order_changed; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER on_order_changed AFTER INSERT OR UPDATE ON public.orders FOR EACH ROW EXECUTE FUNCTION public.update_client_stats();


--
-- Name: reviews on_review_added; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER on_review_added AFTER INSERT ON public.reviews FOR EACH ROW EXECUTE FUNCTION public.update_product_rating();
...
--
-- Name: addresses addresses_user_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.addresses
    ADD CONSTRAINT addresses_user_fk FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
...
--
-- Name: events events_user_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.events
    ADD CONSTRAINT events_user_fk FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
--
-- Name: favorites favorites_product_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.favorites
    ADD CONSTRAINT favorites_product_fk FOREIGN KEY (product_id) REFERENCES public.products(id) ON DELETE CASCADE;


--
-- Name: favorites favorites_user_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.favorites
    ADD CONSTRAINT favorites_user_fk FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.orders
    ADD CONSTRAINT orders_user_fk FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
--
-- Name: products products_category_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.products
    ADD CONSTRAINT products_category_fk FOREIGN KEY (category_id) REFERENCES public.categories(id);
...
--
-- Name: support_tickets support_tickets_user_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.support_tickets
    ADD CONSTRAINT support_tickets_user_fk FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
--
-- Name: user_carts user_carts_user_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.user_carts
    ADD CONSTRAINT user_carts_user_fk FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;


--
-- Name: user_carts user_carts_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: banners Admin full access banners; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: app_settings Admin update settings; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admin update settings" ON public.app_settings FOR UPDATE TO authenticated USING (true);


--
-- Name: coupon_usage Admin view usage; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admin view usage" ON public.coupon_usage FOR SELECT TO authenticated USING (true);


--
-- Name: products Allow anon insert on products; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Allow anon insert on products" ON public.products FOR INSERT TO anon WITH CHECK (true);


--
-- Name: products Allow authenticated insert on products; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: wishlist Allow public delete wishlist; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: wishlist Allow public insert wishlist; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: wishlist Allow public select wishlist; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: products Enable all access for authenticated users; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: banners Public Read Banners; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: seo_pages Public read SEO pages; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Public read SEO pages" ON public.seo_pages FOR SELECT USING (true);


--
-- Name: reviews Public reviews insert; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: reviews Public reviews select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: banners Public view active banners; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: app_settings Public view settings; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Public view settings" ON public.app_settings FOR SELECT USING (true);


--
-- Name: events User can insert own events; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "User can insert own events" ON public.events FOR INSERT WITH CHECK (((auth.uid() = user_id) OR (user_id IS NULL)));


--
-- Name: user_carts User can manage own cart; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "User can manage own cart" ON public.user_carts USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));


--
-- Name: events User can read own events; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "User can read own events" ON public.events FOR SELECT USING ((auth.uid() = user_id));


--
-- Name: addresses User manages own addresses; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "User manages own addresses" ON public.addresses USING ((auth.uid() = user_id));
...
CREATE POLICY admin_manage_categories ON public.categories USING ((EXISTS ( SELECT 1
...
--
-- Name: admins admins_own_row; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY admins_own_row ON public.admins FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY categories_admin_all ON public.categories USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: categories categories_public_select; Type: POLICY; Schema: public; Owner: -
--
...
CREATE POLICY clients_admin_all ON public.clients USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
CREATE POLICY clients_admin_select ON public.clients FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));


--
-- Name: clients clients_public_insert; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: coupon_usage coupon_usage_admin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY coupon_usage_admin_all ON public.coupon_usage USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: coupon_usage coupon_usage_admin_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY coupon_usage_admin_insert ON public.coupon_usage FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));


--
-- Name: coupon_usage coupon_usage_user_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY coupon_usage_user_select ON public.coupon_usage FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.orders o
  WHERE ((o.id = coupon_usage.order_id) AND (o.user_id = auth.uid())))));
...
CREATE POLICY coupons_admin_all ON public.coupons USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: coupons coupons_public_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: favorites favorites_admin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY favorites_admin_all ON public.favorites USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: favorites favorites_user_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY favorites_user_all ON public.favorites USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
...
--
-- Name: home_sections home_sections_admin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY home_sections_admin_all ON public.home_sections USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: home_sections home_sections_public_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY home_sections_public_select ON public.home_sections FOR SELECT USING (true);
...
CREATE POLICY order_items_admin_all ON public.order_items TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: order_items order_items_anon_whatsapp_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY order_items_anon_whatsapp_all ON public.order_items TO anon USING ((EXISTS ( SELECT 1
   FROM public.orders o
  WHERE ((o.id = order_items.order_id) AND (o.user_id IS NULL) AND (o.platform = 'whatsapp'::text))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.orders o
  WHERE ((o.id = order_items.order_id) AND (o.user_id IS NULL) AND (o.platform = 'whatsapp'::text)))));


--
-- Name: order_items order_items_user_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY order_items_user_all ON public.order_items TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.orders o
...
CREATE POLICY orders_admin_all ON public.orders TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: orders orders_anon_whatsapp_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY orders_anon_whatsapp_insert ON public.orders FOR INSERT TO anon WITH CHECK (((platform = 'whatsapp'::text) AND (user_id IS NULL)));


--
-- Name: orders orders_anon_whatsapp_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY orders_anon_whatsapp_select ON public.orders FOR SELECT TO anon USING (((platform = 'whatsapp'::text) AND (user_id IS NULL)));


--
-- Name: orders orders_auth_user_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY orders_auth_user_all ON public.orders TO authenticated USING ((user_id = auth.uid())) WITH CHECK ((user_id = auth.uid()));
...
--
-- Name: profiles profiles_user_own_row; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY profiles_user_own_row ON public.profiles FOR SELECT USING ((auth.uid() = id));


--
-- Name: profiles profiles_user_update_own_row; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY profiles_user_update_own_row ON public.profiles FOR UPDATE USING ((auth.uid() = id)) WITH CHECK ((auth.uid() = id));
...
--
-- Name: reviews reviews_public_approved_only; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY reviews_public_approved_only ON public.reviews FOR SELECT USING ((is_approved = true));
...
--
-- Name: user_carts; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.user_carts ENABLE ROW LEVEL SECURITY;

--
-- Name: user_carts user_carts_admin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_carts_admin_all ON public.user_carts USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: user_carts user_carts_user_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_carts_user_all ON public.user_carts USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
...
--
-- Name: wishlist wishlist_admin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY wishlist_admin_all ON public.wishlist USING ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))));
...
--
-- Name: wishlist wishlist_user_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY wishlist_user_all ON public.wishlist USING (((auth.jwt() ->> 'email'::text) = user_email)) WITH CHECK (((auth.jwt() ->> 'email'::text) = user_email));
...
--
-- Name: objects Admin Upload Banners; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Admin Upload Banners" ON storage.objects FOR INSERT TO authenticated WITH CHECK ((bucket_id = 'banners'::text));


--
-- Name: objects Admins manage banners; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Admins manage banners" ON storage.objects USING (((bucket_id = 'banners'::text) AND (EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))))) WITH CHECK (((bucket_id = 'banners'::text) AND (EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))));


--
-- Name: objects Admins manage product images; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Admins manage product images" ON storage.objects USING (((bucket_id = 'products'::text) AND (EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid()))))) WITH CHECK (((bucket_id = 'products'::text) AND (EXISTS ( SELECT 1
   FROM public.admins a
  WHERE (a.user_id = auth.uid())))));


--
-- Name: objects Allow All Access 1ifhysk_0; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Allow All Access 1ifhysk_0" ON storage.objects FOR SELECT TO anon USING ((bucket_id = 'products'::text));


--
-- Name: objects Allow All Access 1ifhysk_1; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Allow All Access 1ifhysk_1" ON storage.objects FOR INSERT TO anon WITH CHECK ((bucket_id = 'products'::text));


--
-- Name: objects Allow All Access 1ifhysk_2; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Allow All Access 1ifhysk_2" ON storage.objects FOR UPDATE TO anon USING ((bucket_id = 'products'::text));
...
CREATE POLICY "Allow Deletes" ON storage.objects FOR DELETE TO authenticated USING ((bucket_id = 'products'::text));


--
-- Name: objects Allow Updates; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Allow Updates" ON storage.objects FOR UPDATE TO authenticated USING ((bucket_id = 'products'::text));
...
CREATE POLICY "Allow Uploads" ON storage.objects FOR INSERT TO authenticated WITH CHECK ((bucket_id = 'products'::text));
...
--
-- Name: objects Public Access Banners; Type: POLICY; Schema: storage; Owner: -
--
...
--
-- Name: objects Public read banners; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "Public read banners" ON storage.objects FOR SELECT USING ((bucket_id = 'banners'::text));
...
CREATE POLICY "Public read product images" ON storage.objects FOR SELECT USING ((bucket_id = 'products'::text));


--
-- Name: objects User manage own avatar; Type: POLICY; Schema: storage; Owner: -
--

CREATE POLICY "User manage own avatar" ON storage.objects USING (((bucket_id = 'avatars'::text) AND (owner = auth.uid()))) WITH CHECK (((bucket_id = 'avatars'::text) AND (owner = auth.uid())));
...
--
-- Name: supabase_realtime banners; Type: PUBLICATION TABLE; Schema: public; Owner: -
--
...
\unrestrict yxzl6g4fVBWGBwHfRqCcZYEITav0TmSKqMYrCBrWaBICyWeY9VvXc56PK9oUTzk
