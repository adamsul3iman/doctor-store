-- ============================================
-- Analytics Tables for Doctor Store (CLEAN VERSION)
-- ============================================

-- Drop existing functions first
DROP FUNCTION IF EXISTS get_dashboard_stats();
DROP FUNCTION IF EXISTS get_top_products();
DROP FUNCTION IF EXISTS get_analytics_for_days();
...
CREATE TABLE IF NOT EXISTS product_views (
  ...
  user_id UUID REFERENCES auth.users(id),
  category_id TEXT, -- Changed to TEXT to match products.category type
...
SELECT 
  ...
  p.image_url,
  p.category,
  p.category as category_name, -- Since category is TEXT, use it directly
  pa.total_views,
  pa.unique_viewers,
  pa.cart_additions,
  pa.purchases,
...
-- فقط الأدمن يمكنه رؤية جميع البيانات (باستخدام user_metadata.is_admin)
CREATE POLICY admin_events_policy ON events
  FOR ALL TO authenticated
  USING (auth.jwt() ->> 'role' = 'authenticated' AND auth.jwt() ->> 'email' IN (
...
-- دالة للحصول على المنتجات الأكثر مشاهدة
...
BEGIN
  ...
  SELECT 
    ...
    p.title,
    pa.total_views,
    p.category, -- Use category directly since it's TEXT
    p.image_url
  FROM products p
  JOIN product_analytics pa ON p.id = pa.product_id
  WHERE p.is_active = true
