...
class SmartSearchService {
  ...
  final Map<String, Future<List<Product>>> _inFlight = {};
  ...
  static const Map<String, List<String>> _synonyms = {
    ...
    'buy one get one': [
      'اشتري واحد واحصل على واحد مجاني',
      'bogo',
      'عرض الشراء',
      '1+1'
    ],
  ...
  Future<List<Product>> smartSearch(String query) async {
    ...
    final cachedAt = _cacheTime[trimmed];
    ...
    try {
      final future = () async {
        ...
        final finalResults = results.isEmpty
            ? _sortByRelevance(
                await _fuzzySearch(correctedQuery),
                correctedQuery,
                synonyms,
              )
            : _sortByRelevance(results, correctedQuery, synonyms);
  ...
  List<String> _getSynonyms(String word) {
    final cached = _synonymsCache[word];
    ...
    if (_synonymsCache.length >= 200) {
      _synonymsCache.clear();
    }
    _synonymsCache[word] = list;
    return list;
  }

  /// البحث المتعدد في العنوان والوصف والمرادفات
  Future<List<Product>> _multiSearch(
      String query, List<String> synonyms) async {
    final Set<Product> uniqueProducts = {};

    // البحث بجميع الكلمات (الأصلية + المرادفات) بشكل متوازي
    final allTerms = <String>{query, ...synonyms};

    final terms = allTerms.toList();
    final futures = terms.map(_searchByTerm).toList();
  ...
  Future<List<Product>> _multiSearchPaginated(
    ...
    final futures = allTerms
        .map((term) => _searchByTermPaginated(
              term,
              page: page,
              pageSize: pageSize,
            ))
    ...
    final allResults =
        _sortByRelevance(uniqueProducts.toList(), query, synonyms);
